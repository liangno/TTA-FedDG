...
from utils.fed_merge import Cal_Weight_Dict, FedUpdate,FedAvg
...
def get_argparse():
    ...
    parser.add_argument('--comm', help='epochs number', type=int, default=2)
...
def main():
    ...
    args = get_argparse()
    log_dir, tensorboard_dir,save_dir = Gen_Log_Dir(args, file_name=file_name)
    ...
    for i in range(args.comm+1):
        ...
        for domain_name in dataobj.train_domain_list:
            ...
            '''只保存一个最终模型文件，以测试域命名'''
        print(save_dir)
        output_path =  os.path.join(save_dir,f"{args.test_domain}.pt")
        torch.save({'state_dict': global_model.state_dict()}, output_path)

        #torch.save(global_model.state_dict(), output_path)
        log_file.info(f"Saved final model as {output_path}")

        log_file.info("Training completed.")
